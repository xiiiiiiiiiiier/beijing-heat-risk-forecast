...
def make_raw(unit_id, temperature=34.0):
    units, coordinates = load_city("tangshan")
...
class TangshanPipelineTest(unittest.TestCase):
    def test_entry_imports_from_repository_root(self):
        completed = subprocess.run(
            [sys.executable, "-c", "from scripts.update_all import run_tangshan_forecast"],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
    ...
    def test_recognizes_exactly_the_4_at_units(self):
        self.assertEqual(update_all.get_tangshan_unit_ids(), EXPECTED_UNIT_IDS)
        for invalid in ("tangshan_urban", "tangshan_other", "tangshan_missing"):
            with self.subTest(invalid=invalid):
                with self.assertRaisesRegex(ValueError, "Unsupported Tangshan unit_id"):
                    update_all.run_tangshan_forecast(
                        unit_id=invalid, raw=pd.DataFrame(), save=False
                    )

    def test_core_runs_hourly_daily_at_and_both_age_risks(self):
        result = update_all.run_tangshan_forecast(
            unit_id="tangshan_core", raw=make_raw("tangshan_core"), save=False
        )
    ...
    def test_guye_runs_through_the_same_entry_with_9_points(self):
        raw = make_raw("tangshan_130204")
        self.assertEqual(len(raw), 168 * 9)
        result = update_all.run_tangshan_forecast(
            unit_id="tangshan_130204",
            raw=raw,
            save=False,
        )
        self.assertEqual(len(result), 7)
    ...
    def test_new_entry_reproduces_committed_core_results(self):
        hourly = pd.read_csv(ROOT / "data/current/units/tangshan_core_hourly.csv")
        expected_daily = pd.read_csv(
            ROOT / "data/current/units/tangshan_core_daily.csv"
        )
        expected_risk = pd.read_csv(
            ROOT / "data/processed/units/tangshan_core_health_risk.csv"
        )[RESULT_COLUMNS]

        actual_risk = update_all.run_tangshan_forecast(
            unit_id="tangshan_core",
            raw=raw_from_weighted_hourly("tangshan_core", hourly),
            save=False,
        )
    ...
    def test_existing_batch_cli_still_dispatches_tangshan(self):
        with (
            patch.object(sys, "argv", ["update_units.py", "--city", "tangshan"]),
            patch.object(update_units, "update_city") as update_city,
        ):
            self.assertEqual(update_units.main(), 0)
        update_city.assert_called_once_with("tangshan")
